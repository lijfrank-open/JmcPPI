...
def load_data(dataset, split_mode, seed, skip_head=True):
    ...
    ppi_path = '/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/protein.actions.{}.txt'.format(dataset)
    prot_seq_path = '/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/protein.{}.sequences.dictionary.csv'.format(
        dataset)
    prot_r_edge_path = '/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/protein.rball.edges.{}.npy'.format(
        dataset)
    prot_k_edge_path = '/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/protein.knn.edges.{}.npy'.format(
        dataset)
    prot_node_path = '/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/protein.nodes.{}.pt'.format(
        dataset)

    if os.path.exists("/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/{}_ppi.pkl".format(dataset)):
        with open("/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/{}_ppi.pkl".format(dataset),
                  "rb") as tf:
            ppi_list = pickle.load(tf)
        with open("/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/{}_ppi_label.pkl".format(dataset),
                  "rb") as tf:
    ...
    protein_data = ProteinDatasetDGL(prot_r_edge_path, prot_k_edge_path,
                                     prot_node_path, dataset)
    ppi_split_dict = split_dataset(ppi_list, dataset, split_mode, seed)

    return protein_data, ppi_g.to(device), ppi_list, torch.FloatTensor(
        np.array(ppi_label_list)).to(device), ppi_split_dict


class ProteinDatasetDGL(torch.utils.data.Dataset):

    def __init__(self, prot_r_edge_path, prot_k_edge_path, prot_node_path,
                 dataset):

        if os.path.exists(
                "../data/processed_data/{}_protein_graphs.pkl".format(
                    dataset)):
            with open(
                    "../data/processed_data/{}_protein_graphs.pkl".format(
                        dataset), "rb") as tf:
...
def split_dataset(ppi_list, dataset, split_mode, seed):
    if not os.path.exists("/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/{}_{}.json".format(
            dataset, split_mode)):
        if split_mode == 'random':
            ...
            ppi_split_dict['val_index'] = random_list[int(ppi_num *
                                                          0.6):int(ppi_num *
                                                                   0.8)]
            ...
            with open(
                    "/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/{}_{}.json".format(
                        dataset, split_mode), 'w') as f:
                f.write(jsobj)
                f.close()
            ...
            if split_mode == 'bfs':
                selected_edge_index = get_bfs_sub_graph(
                    ppi_list, node_num, node_to_edge_index, sub_graph_size)
            elif split_mode == 'dfs':
                selected_edge_index = get_dfs_sub_graph(
                    ppi_list, node_num, node_to_edge_index, sub_graph_size)

            all_edge_index = [i for i in range(ppi_num)]
            unselected_edge_index = list(
                set(all_edge_index).difference(set(selected_edge_index)))
            ...
            ppi_split_dict['val_index'] = [
                ...
                for i in random_list[:int(ppi_num * 0.2)]
            ]
            ...
            print("your mode is {}, you should use bfs, dfs or random".format(
                split_mode))
            return
    else:
        with open(
                "/home/lijfrank/code/dataset/STRING_27k_148k/processed_data/{}_{}.json".format(
                    dataset, split_mode), 'r') as f:
            ppi_split_dict = json.load(f)
            f.close()

    print("Train_PPI: {} | Val_PPI: {} | Test_PPI: {}".format(
        len(ppi_split_dict['train_index']), len(ppi_split_dict['val_index']),
        len(ppi_split_dict['test_index'])))
